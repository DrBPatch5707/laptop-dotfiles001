...
class SettingsDialog extends JDialog {
    ...
    private void initComponents() {
        ...
        idePanel.add(new JLabel("Available IDEs:"), BorderLayout.NORTH);
        ideListArea = new JTextArea();
        ideListArea.setEditable(false);
        ideListArea.setFont(new Font("Monospaced", Font.PLAIN, 12));
        JScrollPane ideScrollPane = new JScrollPane(ideListArea);
    ...
    private void loadCurrentSettings() {
        ...
        // Load IDE list
        StringBuilder ideInfo = new StringBuilder();
        java.util.Map<String, IDE> ides = BackendAPI.getIdes();
        if (ides != null && !ides.isEmpty()) {
            ideInfo.append("Configured IDEs:\n\n");
            for (java.util.Map.Entry<String, IDE> entry : ides.entrySet()) {
                IDE ide = entry.getValue();
                ideInfo.append("Name: ").append(ide.getName()).append("\n");
                ideInfo.append("Path: ").append(ide.getPath()).append("\n");
                if (ide.getArgs() != null && !ide.getArgs().isEmpty()) {
                    ideInfo.append("Args: ").append(ide.getArgs()).append("\n");
                }
                ideInfo.append("---\n");
            }
        } else {
            ideInfo.append("No IDEs configured.");
        }
        ideListArea.setText(ideInfo.toString());
    }
