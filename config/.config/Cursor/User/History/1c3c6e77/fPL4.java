...
class SettingsDialog extends JDialog {
    ...
    private void loadCurrentSettings() {
        ...
        ideListModel.clear(); // Clear existing items
    ...
    private void editIDE(JList<String> list, DefaultListModel<String> model) {
        ...
        IDE ide = BackendAPI.getIdes().get(selectedName);
        if (ide == null) {
            JOptionPane.showMessageDialog(this, "IDE not found.");
            return;
        }
        ...
        if (newName != null && !newName.trim().isEmpty()) {
            if (BackendAPI.getIdes().containsKey(newName)) {
                JOptionPane.showMessageDialog(this, "IDE with name '" + newName + "' already exists.");
                return;
            }
            BackendAPI.removeIDE(selectedName);
            ide.setName(newName);
            BackendAPI.addIDE(ide);
