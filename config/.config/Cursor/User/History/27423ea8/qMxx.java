...
public class JacksonIdeConfigManager {
    ...
    public static Config loadConfiguration() throws IOException {
        ...
        if (!configFile.exists() || configFile.length() == 0) {
            if (Config.DEBUG) System.out.println("JSON config file not found or empty. Creating default.");
        ...
        if (Config.DEBUG) System.out.println("Loading JSON config from: " + configFilePath);
    ...
    public static void saveConfiguration(Config config) throws IOException {
        ...
        if (Config.DEBUG) System.out.println("Saved JSON config to: " + configFilePath);
    }
    //endregion
