...
public class BackendAPI {
    ...
    public static boolean addProject(Project project, boolean skipDirCreate) {
        if (Config.DEBUG) System.out.println("addProject called...");
        String relPath = normalizeRelativePath(project.getRelativePath());
        if (relPath == null || relPath.trim().isEmpty()) {
            if (Config.DEBUG) System.err.println("Project relative path is null or empty.");
            return false;
        }
    ...
    public static boolean accessProject(Project project, IDE selectedIDE) {
        try {
            ...
            if (Config.DEBUG) System.err.println("Error accessing project: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
        System.exit(0);
    ...
    public static void updateProjectLastModified(Project project) {
        try {
            ...
            if (Config.DEBUG) System.out.println("Updated last modified date for project '" + project.getName() + "': " + lastModifiedDate);
        } catch (Exception e) {
            if (Config.DEBUG) System.err.println("Error updating last modified date for project '" + project.getName() + "': " + e.getMessage());
        }
    }
    ...
    private static String getGitLastCommitDate(String projectPath) {
        try {
            ...
            if (Config.DEBUG) System.err.println("Error getting git commit date: " + e.getMessage());
            return null;
        }
    }
    ...
    public static boolean promptForRoot() {
        ...
        if (temp == null || temp.trim().isEmpty()) {
            if (Config.DEBUG) System.out.println("User cancelled or entered empty root directory");
            return false;
        }
        ...
        if (Files.isDirectory(Paths.get(temp))) {
            ...
            if (success) {
                if (Config.DEBUG) System.out.println("Root directory set successfully to: " + root);
                return true;
            } else {
                if (Config.DEBUG) System.out.println("Failed to save root directory to file");
                return false;
            }
        } else {
            if (Config.DEBUG) System.out.println("Invalid directory path: " + temp);
    ...
    public static void scanForUnregisteredProjects() {
        ...
        try {
            ...
            if (Config.DEBUG) System.err.println("Error scanning filesystem for unregistered projects: " + e.getMessage());
        }
    }
