...
public class BackendAPI {
    ...
    public static boolean accessProject(Project project, IDE selectedIDE) {
        try {
            ...
            if (ideName.contains("code") || ideName.contains("vscode") || selectedIDE.getPath().contains("code")) {
                // VS Code: use absolute path and ensure it's treated as a folder
                command.add(projectPath);
                // Add VS Code specific arguments if none are set
                if (selectedIDE.getArgs() == null || selectedIDE.getArgs().trim().isEmpty()) {
                    command.add("--new-window");
                }
                
                // Alternative: If the above doesn't work, try using --folder-uri format
                // Uncomment the following lines if VS Code still opens as file:
                /*
                command.clear();
                command.add(selectedIDE.getPath());
                command.add("--folder-uri");
                command.add("file://" + projectPath);
                if (selectedIDE.getArgs() == null || selectedIDE.getArgs().trim().isEmpty()) {
                    command.add("--new-window");
                }
                */
            } else {
                // Other IDEs: use the project path as is
                command.add(projectPath);
            }
            ...
            System.out.println("Project exists: " + Files.exists(Paths.get(projectPath)));
            System.out.println("Project is directory: " + Files.isDirectory(Paths.get(projectPath)));
            System.out.println("========================");
