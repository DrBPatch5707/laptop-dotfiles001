...
public class init {
    ...
    private static void scanForUnregisteredProjects(java.nio.file.Path dir, String relativePath, 
        ...
        try {
            ...
            java.util.List<java.nio.file.Path> entryList = entries.collect(java.util.stream.Collectors.toList());
            for (java.nio.file.Path entry : entryList) {
    ...
    private static boolean appearsToBeProject(java.nio.file.Path dir) {
        try {
            ...
            // If it's a leaf directory (no subdirectories), it might be a simple or new project
            if (dirCount == 0) {
                if (fileCount > 0) {
                    ...
                    return true; // Accept empty leaf directories as new projects
                }
            }
