...
class SettingsDialog extends JDialog {
    ...
    private void changeRoot() {
        ...
        String warningMsg = "Changing the root directory will RESET the database and DELETE ALL PROJECT INFORMATION (but NOT the actual project files).\nAre you sure you want to continue?";
        int confirm = JOptionPane.showConfirmDialog(this, warningMsg, "Confirm Root Change", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (confirm != JOptionPane.YES_OPTION) return;
        boolean success = BackendAPI.promptForRoot();
        String newRoot = BackendAPI.root;
        if (success) {
            if (oldRoot == null || !oldRoot.equals(newRoot)) {
                BackendAPI.resetDatabase();
                currentRootLabel.setText(newRoot);
                JOptionPane.showMessageDialog(this, "Root directory updated and database reset successfully!");
            } else {
                currentRootLabel.setText(newRoot);
                JOptionPane.showMessageDialog(this, "Root directory unchanged. No reset performed.");
            }
        } else {
    ...
    public void initSetup() {
        ...
        "Welcome to Project Manager!\n\nIt looks like this is your first time setting up the application, or you have just performed a full reset. " +
        "You may need to configure your root directory and review detected projects.\n\n" +
        "Please follow the prompts carefully to ensure your environment is set up correctly.",
        "First-Time Setup Notice",
        JOptionPane.INFORMATION_MESSAGE
    );
    // Prompt for new root
    org.drbpatch.Settings.getRoot();
    loadCurrentSettings();
    // Run all initialization syncs and checks
    org.drbpatch.ProjectSync.performFilesystemSync();
    // Reload project table in all GUIs
    org.drbpatch.ProjectManagerGUI.reloadProjectsInAllGUIs();
    JOptionPane.showMessageDialog(this, "Database and root path reset successfully!", "Reset Complete", JOptionPane.INFORMATION_MESSAGE);
    }

    // Add this method inside SettingsDialog
    private void resetDatabaseAndRoot() {
        int confirm = JOptionPane.showConfirmDialog(this,
                "This will DELETE ALL PROJECTS and RESET the root directory.\nAre you sure you want to continue?",
                "Confirm Reset", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (confirm != JOptionPane.YES_OPTION) return;
        boolean dbDeleted = false, rootDeleted = false;
        try {
            // Close DB connection
            ...
            java.nio.file.Path dbPath = java.nio.file.Paths.get("projects.db");
            if (java.nio.file.Files.exists(dbPath)) {
                java.nio.file.Files.delete(dbPath);
                dbDeleted = true;
            } else {
                dbDeleted = true; // If not exist, treat as deleted
            }
            // Delete root.txt
            java.nio.file.Path rootPath = org.drbpatch.Settings.rootConfPath;
            if (java.nio.file.Files.exists(rootPath)) {
                java.nio.file.Files.delete(rootPath);
                rootDeleted = true;
            } else {
                rootDeleted = true;
            }
            // Reinitialize DB
            Main.connection = org.drbpatch.DbConnection.connect();
            org.drbpatch.DbConnection.createProjectsTable(Main.connection);
            org.drbpatch.BackendAPI.projects.clear();
            // Clear skipped directories for this run
            org.drbpatch.ProjectSync.clearSkippedThisRun();
            // Show professional first-time setup warning
            initSetup();
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Failed to reset database or root: " + ex.getMessage(), "Reset Failed", JOptionPane.ERROR_MESSAGE);
        }
    }
} 
